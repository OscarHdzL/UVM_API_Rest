    ...
    public class CatCampusNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatCampuses.FromSqlInterpolated($@"EXEC sp_Campus_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> GetById(int id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatCampuses.FromSqlInterpolated($@"EXEC sp_Campus_Select @Id = {id}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
                Respuesta = TipoAccion.Positiva(resultados);
