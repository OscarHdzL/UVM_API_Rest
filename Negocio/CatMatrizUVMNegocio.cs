...
namespace Negocio
{
    public class CatMatrizUVMNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                List<CatMatrizUvm> lista = new List<CatMatrizUvm>();


                if (pageSize == 0)
                    throw new Exception("El parámetro pageSize debe ser mayor a cero");

                var resultados = await ctx.VwCatMatrizUvms.FromSqlInterpolated($@"EXEC sp_MatrizUVM_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> GetById(int id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.VwCatMatrizUvms.FromSqlInterpolated($@"EXEC sp_MatrizUVM_Select @Id = {id}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
                Respuesta = TipoAccion.Positiva(resultados);
        ...
        public async Task<TipoAccion> Insertar(MatrizUVMDTO entidad)
        {
            try
            {
        ...
        public async Task<TipoAccion> Actualizar(MatrizUVMDTO entidad)
        {
            try
            {
                var parametroId = new SqlParameter("@idRespuesta", SqlDbType.Int);
        ...
        public async Task<TipoAccion> Deshabilitar(int id)
        {
            try
            {
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_MatrizUVM_Actualiza
                                    @TipoActualiza = {"D"}, @Id = {id}, @ComponenteUvmID = {null}, @Activo = {null}, @FechaCreacion = {null}, 
