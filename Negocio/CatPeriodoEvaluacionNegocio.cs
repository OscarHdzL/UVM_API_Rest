    ...
    public class CatPeriodoEvaluacionNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatPeriodoEvaluacions.FromSqlInterpolated($@"EXEC sp_PeriodoEvaluacion_Select @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
