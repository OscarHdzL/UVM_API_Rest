...
namespace Negocio
{
    public class SubIndicadorUvmNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                List<SubIndicadorUvmDTO> lista = new List<SubIndicadorUvmDTO>();


                if (pageSize == 0)
                    throw new Exception("El parámetro pageSize debe ser mayor a cero");

                var resultados = await ctx.VwSubIndicadorUvms.FromSqlInterpolated($@"EXEC sp_SubIndicadorUvm_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> GetById(int id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.VwSubIndicadorUvms.FromSqlInterpolated($@"EXEC sp_SubIndicadorUvm_Select @Id = {id}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
                Respuesta = TipoAccion.Positiva(resultados);
        ...
        public async Task<TipoAccion> Insertar(SubIndicadorUvmDTO entidad)
        {
            try
            {
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_SubIndicadorUvm_Actualiza
                                    @TipoActualiza = {"I"}, @Id = {0}, @IndicadorUvmId = {entidad.IndicadorUvmId}, @NombreSubIndicadorUvm = {entidad.NombreSubIndicadorUvm}, @Activo = {entidad.Activo}, @FechaCreacion = {entidad.FechaCreacion}, 
        ...
        public async Task<TipoAccion> Actualizar(SubIndicadorUvmDTO entidad)
        {
            try
            {
                var parametroId = new SqlParameter("@idRespuesta", SqlDbType.Int);
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_SubIndicadorUvm_Actualiza
                                    @TipoActualiza = {"U"}, @Id = {entidad.Id}, @IndicadorUvmId = {entidad.IndicadorUvmId}, @NombreSubIndicadorUvm = {entidad.NombreSubIndicadorUvm}, @Activo = {entidad.Activo}, @FechaCreacion = {entidad.FechaCreacion}, 
                                    @UsuarioCreacion = {entidad.UsuarioCreacion}, @FechaModificacion = {entidad.FechaModificacion}, 
                                    @UsuarioModificacion = {entidad.UsuarioModificacion}, 
                                    @idRespuesta = {parametroId} OUTPUT, @exito = {parametroExito} OUTPUT,  @mensaje = {parametroMensaje} OUTPUT");
        ...
        public async Task<TipoAccion> Deshabilitar(int id)
        {
            try
            {
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_SubIndicadorUvm_Actualiza
                                    @TipoActualiza = {"D"}, @Id = {id}, @IndicadorUvmId = {null}, @NombreSubIndicadorUvm = {null}, @Activo = {null}, @FechaCreacion = {null}, 
