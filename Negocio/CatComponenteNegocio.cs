...
namespace Negocio
{
    public class CatComponenteNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                List<CatComponente> lista = new List<CatComponente>();


                if (pageSize == 0)
                    throw new Exception("El parámetro pageSize debe ser mayor a cero");

                var resultados = await ctx.CatComponentes.FromSqlInterpolated($@"EXEC sp_Componente_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> GetById(int id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatComponentes.FromSqlInterpolated($@"EXEC sp_Componente_Select @Id = {id}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
                Respuesta = TipoAccion.Positiva(resultados);
        ...
        public async Task<TipoAccion> Insertar(CatComponente entidad)
        {
            try
            {
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_Componente_Actualiza
                                    @TipoActualiza = {"I"}, @Id = {0}, @Clave = {entidad.Clave}, @Nombre = {entidad.Nombre}, @Activo = {entidad.Activo}, @FechaCreacion = {entidad.FechaCreacion}, 
        ...
        public async Task<TipoAccion> Actualizar(CatComponente entidad)
        {
            try
            {
                var parametroId = new SqlParameter("@idRespuesta", SqlDbType.Int);
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_Componente_Actualiza
                                    @TipoActualiza = {"U"}, @Id = {entidad.Id}, @Clave = {entidad.Clave}, @Nombre = {entidad.Nombre}, @Activo = {entidad.Activo}, @FechaCreacion = {entidad.FechaCreacion}, 
                                    @UsuarioCreacion = {entidad.UsuarioCreacion}, @FechaModificacion = {entidad.FechaModificacion}, 
                                    @UsuarioModificacion = {entidad.UsuarioModificacion}, 
                                    @idRespuesta = {parametroId} OUTPUT, @exito = {parametroExito} OUTPUT,  @mensaje = {parametroMensaje} OUTPUT");
