    ...
    public class CatIndicadorSIACNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatIndicadorSiacs.FromSqlInterpolated($@"EXEC sp_IndicadorSIAC_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> GetById(int id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.CatIndicadorSiacs.FromSqlInterpolated($@"EXEC sp_IndicadorSIAC_Select @Id = {id}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
                Respuesta = TipoAccion.Positiva(resultados);
