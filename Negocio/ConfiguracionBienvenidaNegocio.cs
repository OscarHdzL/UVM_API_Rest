...
namespace Negocio
{
    public class ConfiguracionBienvenidaNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try
            {
                ...
                var resultados = await ctx.ConfiguracionBienvenida.FromSqlInterpolated($@"EXEC sp_ConfiguracionBienvenida_Select @Id = {null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
        ...
        public async Task<TipoAccion> Actualizar(ConfiguracionBienvenidum entidad)
        {
            try
            {
                ...
                await ctx.Database.ExecuteSqlInterpolatedAsync($@"EXEC sp_ConfiguracionBienvenida_Actualiza
                                    @TipoActualiza = {"U"},
