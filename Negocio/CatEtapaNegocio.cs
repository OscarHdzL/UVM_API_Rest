    ...
    public class CatEtapaNegocio
    {
        ...
        public async Task<TipoAccion> Get(int? id, int pageSize, int pageNumber)
        {
            try { 
                ...
                var resultados = await ctx.VwCatEtapas.FromSqlInterpolated($@"EXEC sp_Etapa_Select @Id={null}, @PageSize = {pageSize}, @PageNumber = {pageNumber}").ToListAsync();
